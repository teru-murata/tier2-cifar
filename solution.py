...
def train(config):
    ...
    if os.path.exists(checkpoint_file):
        if config.resume:
            ...
            load_res = torch.load(checkpoint_file, weights_only=False)
            # Resume iterations
            ...
            if iter_idx % config.val_intv == 0:
                ...
                for data in va_data_loader:
                    ...
                    with torch.no_grad():
                        ...
                        # to aviod TypeError and empty array
                        pred = torch.argmax(logits, dim=1) 
                        acc  = torch.mean((pred == y).float()) * 100.0 
                        va_acc.append(acc.cpu().numpy()) #Just in case...
...
def test(config):
    ...
    model = MyNetwork(
        ...
        mean=x_tr_mean, std=x_tr_std #To pass the same mean and std
        )
    #print(model)
    ...
    # except Exception as error:
    #     print('Caught this error: ' + error)
    bestmodel_file = os.path.join(config.save_dir, "best_model.pth") # Load the best model
    print("Loading:", bestmodel_file) # added for debugging
...
if __name__ == "__main__":
    ...
    #main(config)
    # to distinguish unit 64, 128, 256... (current one overwrites and cause confusion)
    tag = f"u{config.num_unit}_lr{config.learning_rate}"
    
    import sys
    if "--save_dir" not in sys.argv:
        config.save_dir = os.path.join("save", tag)
    if "--log_dir"  not in sys.argv:
        config.log_dir  = os.path.join("logs", tag)
